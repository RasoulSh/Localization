...
namespace TMPro
{
    ...
    /// </summary>
    public class RTL_TMP_InputField : Selectable,
    ...
    IScrollHandler
    {
        ...
        protected virtual void LateUpdate()
        {
            ...
            if (!isFocused && m_SelectionStillActive)
            {
                ...
                if (selectedObject != null && selectedObject != this.gameObject)
                {
                    ...
                    if (selectedObject.GetComponent<RTL_TMP_InputField>() != null)
                        ReleaseSelection();
        ...
        public virtual void OnDrag(PointerEventData eventData)
        {
            if (!MayDrag(eventData))
                return;
            var nearCaretPosition = textComponent.isRightToLeftText ? CaretPosition.Right : CaretPosition.Left;
            var farCaretPosition = textComponent.isRightToLeftText ? CaretPosition.Left : CaretPosition.Right;
        ...
        public override void OnPointerDown(PointerEventData eventData)
        {
            ...
            if (hadFocusBefore || !m_OnFocusSelectAll)
            {
                ...
                Debug.Log($"insertionIndex: {insertionIndex} insertionSide: {insertionSide}" +
                          $" string position: {stringPositionInternal} caretPosition: {caretPositionInternal}");
            ...
            else
            {
                if (m_isRichTextEditingAllowed)
                {
                    if (stringPositionInternal > 0)
                    {
                        ...
                        var i = FindPrevWordBegin();
                        var distanceToPrevWord = stringPositionInternal - i;

                        stringSelectPositionInternal = stringPositionInternal = stringPositionInternal - (ctrl ? distanceToPrevWord  : numberOfCharactersToRemove);

                        m_Text = text.Remove(stringPositionInternal, ctrl ? distanceToPrevWord : numberOfCharactersToRemove);

                        caretSelectPositionInternal = caretPositionInternal = caretPositionInternal - (ctrl ? distanceToPrevWord  : 1);
                ...
                else
                {
                    if (caretPositionInternal > 0)
                    {
                        ...
                        var i = FindPrevWordBegin();
                        var distanceToPrevWord = caretPositionInternal - i;
                        // Delete the previous character
                        m_Text = text.Remove(m_TextComponent.textInfo.characterInfo[caretPositionInternal - (ctrl ? distanceToPrevWord : 1)].index,  (ctrl ? distanceToPrevWord : numberOfCharactersToRemove));

                        // Get new adjusted string position
                        stringSelectPositionInternal = stringPositionInternal = caretPositionInternal < 1
                            ? m_TextComponent.textInfo.characterInfo[0].index
                            : m_TextComponent.textInfo.characterInfo[caretPositionInternal - (ctrl ? distanceToPrevWord : 1)].index;
        ...
        private void GenerateCaret(VertexHelper vbo, Vector2 roundingOffset)
        {
            ...
            if (caretPositionInternal == m_TextComponent.textInfo.lineInfo[currentLine].firstCharacterIndex)
            {
                ...
                if (m_TextComponent.verticalAlignment == VerticalAlignmentOptions.Geometry)
                    startPosition = new Vector2(textComponent.isRightToLeftText ? currentCharacter.topRight.x : currentCharacter.origin, 0 - height / 2);
                else
                    startPosition = new Vector2(textComponent.isRightToLeftText ? currentCharacter.topRight.x : currentCharacter.origin, currentCharacter.descender);
            }
            else
            {
        ...
        private void GenerateHightlight(VertexHelper vbo, Vector2 roundingOffset)
        {
            ...
            {
                caretPosition = new Vector2(textComponent.isRightToLeftText ? textInfo.characterInfo[m_CaretSelectPosition].topRight.x : textInfo.characterInfo[m_CaretSelectPosition].origin, textInfo.characterInfo[m_CaretSelectPosition].descender);
            ...
            while (currentChar <= endChar && currentChar < textInfo.characterCount)
            {
                if (currentChar == nextLineStartIdx || currentChar == endChar)
                {
                    ...
                    Vector2 startPosition = new Vector2(textComponent.isRightToLeftText ? startCharInfo.topRight.x : startCharInfo.origin, textInfo.lineInfo[currentLineIndex].ascender);
